...
<html>
	<head>
		...
		<script type="text/javascript" src="<c:url value="/javascripts/pages/issue/status/closed.js"></c:url>"></script>
		...
		<div id="content">
			<div class="container">
				...
				<div class="edit-form" id="issueEdit">
					...
					<input id="summary" class="text" type="text" name="summary" disabled="disabled" value="${issue.summary}" />
					<label for="description">
						<spring:message code="issueEdit.input.description"></spring:message>
						<span>*</span>
					</label>
					<textarea id="description" class="text" name="description" disabled="disabled">${issue.description}</textarea>
					<label for="status">
					...
					<select id="status" name="status" autocomplete="off">
						<c:forEach items="${statuses}" var="status">
							<c:if test="${status.id eq STATUS_CLOSED or status.id eq STATUS_REOPENED}">
					...
					<select id="resolution" name="resolution" autocomplete="off" disabled="disabled" ${issue.status.id eq STATUS_IN_PROGRESS ? 'disabled' : ''}>
						<c:if test="${empty issue.resolution}">
							<option value="-1" selected="selected"></option>
						</c:if>
						<c:forEach items="${resolutions}" var="resolution">
							<option value="${resolution.id}" ${resolution.id == issue.resolution.id and not empty resolution ? 'selected' : ''}>
								${resolution.name}
							</option>
						</c:forEach>
					</select>
					...
					<select id="type" name="type" disabled="disabled" autocomplete="off">
					...
					<select id="priority" name="priority" disabled="disabled" autocomplete="off">
					...
					<select id="project" name="project" disabled="disabled" autocomplete="off">
					...
					<select id="build" name="build" disabled="disabled" autocomplete="off">
					...
					<select id="assignee" name="assignee" disabled="disabled" autocomplete="off">
