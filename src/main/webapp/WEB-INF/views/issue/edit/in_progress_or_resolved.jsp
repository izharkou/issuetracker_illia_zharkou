...
<html>
	<head>
		...
		<script type="text/javascript" src="<c:url value="/javascripts/pages/issue/status/in_progress.js"></c:url>"></script>
				...
				<div class="edit-form" id="issueEdit">
					...
					<select id="status" name="status" autocomplete="off">
						<c:forEach items="${statuses}" var="status">
							<c:if test="${status.id eq STATUS_IN_PROGRESS or status.id eq STATUS_CLOSED or status.id eq STATUS_RESOLVED}">
					...
					<spring:message code="issueEdit.error.resolution" var="errorResolution"></spring:message>
					<select id="resolution" name="resolution" autocomplete="off" original-title="${errorResolution}" ${issue.status.id eq STATUS_IN_PROGRESS ? 'disabled' : ''}>
