$(function() {
	var form = $("#issueEdit"),
		...
		resolutionLabel = $("label[for='resolution'] span", form),
		type = $("#type", form),
	...
	// enable tipsy
	var inputs = [summary, description, resolution];
	...
	status.bind("change", function(e) {
		if(status.val() == "3") {
			...
			resolutionLabel.addClass("hidden");
			resolution.tipsy("hide");
		} else {
			resolution.removeAttr("disabled");
			resolutionLabel.removeClass("hidden");
		}
	});
	...
	submit.bind("click", function(e) {
		...
		// check resolution
		else if(status.val() != "3" && resolution.val() == "-1") {
			resolution.tipsy("show");
			error = true;
		}
